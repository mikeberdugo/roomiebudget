...
class Apartment(models.Model):
    ...
    name = models.CharField(max_length=100)  # Nombre del apartamento o casa compartida
    type = models.CharField(max_length=20, choices=APARTMENT_TYPES)  # Tipo de vivienda (apartamento, casa, loft, estudio)
    rent_amount = models.DecimalField(max_digits=10, decimal_places=2)  # Canon de arrendamiento mensual
    includes_admin = models.BooleanField(default=False)  # Indica si el valor de administración está incluido en el canon
    admin_fee = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)  # Valor de la administración (opcional si no está incluido en el canon)
    number_of_occupants = models.PositiveIntegerField(default=1)  # Número de habitantes actuales
    number_of_rooms = models.PositiveIntegerField(default=1)  # Número de habitaciones disponibles
    number_of_bathrooms = models.PositiveIntegerField(default=1)  # Número de baños disponibles
    has_parking = models.BooleanField(default=False)  # Indica si la vivienda tiene parqueadero
    members = models.ManyToManyField(User, related_name='apartments')  # Miembros (usuarios) asociados al apartamento

    def __str__(self):
        return self.name

    def add_member(self, user):
        """Agrega un usuario al apartamento."""
        self.members.add(user)

    def remove_member(self, user):
        """Elimina un usuario del apartamento."""
        self.members.remove(user)

    def is_member(self, user):
        """Verifica si un usuario es miembro del apartamento."""
        return self.members.filter(id=user.id).exists()

    @staticmethod
    def get_apartments_by_user(user_id):
        """Obtiene todos los apartamentos asociados a un usuario por su ID."""
        return Apartment.objects.filter(members__id=user_id).order_by('-id')
...
class Service(models.Model):
    name = models.CharField(max_length=100)  # Nombre del servicio (ej. agua, luz, internet)
...
class ServiceBill(models.Model):
    ...
    apartment = models.ForeignKey(Apartment, on_delete=models.CASCADE)  # Apartamento asociado a la factura
    service = models.ForeignKey(Service, on_delete=models.CASCADE)  # Servicio relacionado con la factura
    cost = models.DecimalField(max_digits=10, decimal_places=2)  # Costo mensual del servicio
    start_date = models.DateField()  # Fecha de inicio del ciclo de facturación
    end_date = models.DateField()  # Fecha de finalización del ciclo de facturación
    payment_date = models.DateField()  # Fecha límite de pago del servicio
    type = models.CharField(max_length=20, choices=SERVICE_TYPES, default='fixed')  # Tipo de cálculo del servicio (fijo o por consumo)
    consumption = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)  # Consumo registrado (para servicios por consumo)


    class Meta:
        verbose_name = "Service Bill"
        verbose_name_plural = "Service Bills"
        db_table = 'service_bills'


    def __str__(self):
        return f"{self.service.name} - {self.start_date} a {self.end_date}"



class Roomie(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE)  # Usuario relacionado con el roomie
    name = models.CharField(max_length=100)  # Nombre del roomie
...
class Guest(models.Model):
    name = models.CharField(max_length=100)  # Nombre del invitado
    roomie = models.ForeignKey(Roomie, on_delete=models.CASCADE)  # Roomie que invitó al huésped
    service = models.ForeignKey(Service, on_delete=models.CASCADE)  # Servicio usado durante la estadía del invitado
    arrival_date = models.DateField()  # Fecha de llegada del invitado
    departure_date = models.DateField(null=True, blank=True)  # Fecha de salida del invitado
    ...
    def days_presence(self):
        """Calcula el número de días que el invitado estuvo presente."""
